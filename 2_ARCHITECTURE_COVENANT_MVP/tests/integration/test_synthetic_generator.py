...
def test_generator_creates_complete_validated_artifact_topology(tmp_path: Path) -> None:
    ...
    assert {
        path.name for path in (output / "benchmark").iterdir()
    } == {"cases.json", "qa_pairs.jsonl", "qa_pairs.md"}
...
def test_q_and_a_and_cases_are_cross_referenced_to_golden_artifacts(tmp_path: Path) -> None:
    ...
    qa_records = [
        json.loads(line)
        for line in (output / "benchmark" / "qa_pairs.jsonl").read_text(
            encoding="utf-8"
        ).splitlines()
    ]
    ...
    assert all(
        (output / "covenants" / f"{case['covenant_id']}.json").is_file()
        for case in cases
    )
    assert all(record["answer"] == case["expected"] for case, record in zip(cases, qa_records))
    ...
    borrower_ids = {
        str(row[0].value) for row in workbook["borrowers"].iter_rows(min_row=2)
    }
